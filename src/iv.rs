...
///
/// This module provides a hardware abstraction layer for the LR1110 that properly
...
pub struct Lr1110InterfaceVariant<RESET, BUSY, DIO1, CTRL>
where
    RESET: OutputPin,
    BUSY: InputPin + Wait,
    DIO1: Wait,
...
impl<RESET, BUSY, DIO1, CTRL> Lr1110InterfaceVariant<RESET, BUSY, DIO1, CTRL>
where
    RESET: OutputPin,
    BUSY: InputPin + Wait,
    DIO1: Wait,
    CTRL: OutputPin,
{
    /// Create a new InterfaceVariant for LR1110 with BUSY pin support
    ///
    /// Works on any embedded platform (STM32, NRF52, RP2040, etc.) where the
    /// LR1110's BUSY pin is connected to a GPIO.
    ///
    /// # Arguments
    /// * `reset` - GPIO output pin connected to LR1110 RESET
    /// * `busy` - GPIO input pin connected to LR1110 BUSY (DIO0) - active high when processing
...
impl<RESET, BUSY, DIO1, CTRL> InterfaceVariant for Lr1110InterfaceVariant<RESET, BUSY, DIO1, CTRL>
where
    RESET: OutputPin,
    BUSY: InputPin + Wait,
    DIO1: Wait,
